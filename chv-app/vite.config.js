...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@react-native-community/netinfo': path.resolve(sharedWeb, 'NetInfo.js'),
      '@react-native-community/datetimepicker': path.resolve(sharedWeb, 'DateTimePicker.js'),
    },
  },
  optimizeDeps: {
    noDiscovery: false,
    include: ['react-native-web', '@react-navigation/native'],
  },
  esbuild: {
    loader: { '.js': 'jsx' },
  },
  ...
  server: {
    port: parseInt(process.env.VITE_PORT, 10) || 3003,
    host: true,
    hmr: {
      clientPort: parseInt(process.env.VITE_PORT, 10) || 3003,
      host: 'localhost',
    },
    allowedHosts: true,
  },
