...
export default function ReferralsScreen({ navigation }) {
  ...
  const [wardFacilities, setWardFacilities] = useState({});
  ...
  useEffect(() => {
    Promise.all([
      ...
      const seen = new Set();
      const unique = facs.filter(f => {
        if (seen.has(f.name)) return false;
        seen.add(f.name);
        return true;
      });
      setAllFacilities(unique);
      const byWard = {};
      unique.forEach(f => {
        const w = f.ward;
        if (w) {
          ...
          <View key={p.id} style={styles.motherRow}>
            <View style={styles.motherInfo}>
              ...
              <Text style={styles.optionSub}>
                {p.mother_phone} — {p.ward || 'Ward N/A'} • EDD: {new Date(p.edd_date).toLocaleDateString()}
              </Text>
            </View>
