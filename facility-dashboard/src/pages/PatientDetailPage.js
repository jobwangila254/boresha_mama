...
export default function PatientDetailPage() {
  ...
  async function saveEdit() {
    setSaving(true);
    try {
      const res = await api.updatePregnancy(id, editForm);
      ...
      {editing && (
        <div style={{ ...styles.card, marginBottom: 20, border: '2px solid #2980B9' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div>
              <label style={styles.fieldLabel}>{t('risk_level')}</label>
              <select style={styles.select} value={editForm.riskLevel} onChange={e => setEditForm(f => ({ ...f, riskLevel: e.target.value }))}>
