...
export default function AppointmentsPage() {
  ...
  const tabs = ['scheduled', 'completed', 'cancelled', 'missed', 'all'];
            ...
            <tbody>
              {appointments.map(apt => (
                <tr key={apt.id}>
                  ...
                  <td style={styles.td}>
                    <span style={{ ...styles.statusBadge, background: apt.status === 'scheduled' ? '#EBF5FB' : apt.status === 'completed' ? '#E8F8F5' : '#F0F0F0', color: apt.status === 'scheduled' ? '#2980B9' : apt.status === 'completed' ? '#27AE60' : '#7F8C8D' }}>
                      {apt.status}
                    </span>
                  </td>
                  <td style={styles.td}>
