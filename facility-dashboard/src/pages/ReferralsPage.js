...
export default function ReferralsPage() {
  ...
  useEffect(() => {
    api.getProfile().then(res => {
      setMyFacilityId(res.data?.facilityStaff?.facility_id || null);
    }).catch(() => {});
  }, []);
  ...
  function isReceiver(ref) {
    return myFacilityId && ref.to_facility_id === myFacilityId;
  }

  function isSender(ref) {
    return myFacilityId && ref.from_facility_id === myFacilityId;
  }
            ...
            <tbody>
              {referrals.map(ref => (
                <tr key={ref.id}>
                  ...
                  <td style={styles.td}>
                    {isReceiver(ref) && ref.status === 'pending' && (
                      <>
                    ...
                    {isReceiver(ref) && ref.status === 'accepted' && (
                      <button style={styles.completeActBtn} onClick={() => updateStatus(ref.id, 'completed', 'Treated successfully')}>{t('mark_complete')}</button>
                    )}
                    {isSender(ref) && ref.status === 'pending' && (
                      <button style={styles.cancelBtn} onClick={() => updateStatus(ref.id, 'cancelled', 'Cancelled by sender')}>{t('cancel')}</button>
                    )}
                    {!isReceiver(ref) && !isSender(ref) && ref.status === 'pending' && (
...
const styles = {
  ...
  cancelBtn: { background: '#E74C3C', color: '#fff', border: 'none', padding: '6px 14px', borderRadius: 6, cursor: 'pointer', fontSize: 13, marginRight: 6 },
