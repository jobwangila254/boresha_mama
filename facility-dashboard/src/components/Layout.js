...
export default function Layout({ user, setUser }) {
  ...
  const navItems = [
    ...
    { path: '/patients', label: t('patients'), icon: '👩‍👧' },
    { path: '/appointments', label: t('appointments'), icon: '📅' },
    { path: '/referrals', label: t('referrals'), icon: '🔄' },
  ...
  return (
    <div style={styles.container}>
      <nav style={styles.sidebar}>
        ...
        {user?.facilityStaff?.facility_ward && (
          <div style={styles.wardBadge}>
            <span style={styles.wardIcon}>📍</span>
            {user.facilityStaff.facility_ward} {t('facilities')}
          </div>
        )}
