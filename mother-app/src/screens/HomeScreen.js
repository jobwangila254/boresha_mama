...
export default function HomeScreen({ navigation }) {
  ...
  const loadData = useCallback(async () => {
    try {
      const [pregnancies, apts] = await Promise.all([
        api.getPregnancies(),
        api.getAppointments(),
      ]);
      setAppointments(Array.isArray(apts) ? apts.filter(a => a.status === 'confirmed') : []);
        ...
        <View style={styles.pregnancyCard}>
          ...
          <View style={styles.weekRow}>
            ...
            <View style={styles.dueInfo}>
              ...
              <Text style={styles.dueDate}>{new Date(pregnancy.edd_date).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: 'numeric' })}</Text>
          ...
          {appointments.length > 0 && (
            <TouchableOpacity style={styles.nextAptBanner} onPress={() => navigation.navigate('Appointments')}>
              <Text style={styles.nextAptIcon}>📅</Text>
              <View>
                <Text style={styles.nextAptLabel}>{t('next_appointment')}</Text>
                <Text style={styles.nextAptDate}>{new Date(appointments[0].appointment_date).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: 'numeric' })}</Text>
                <Text style={styles.nextAptType}>{(appointments[0].visit_type || appointments[0].reason || '').replace(/_/g, ' ')}</Text>
              </View>
            </TouchableOpacity>
          )}
          <View style={styles.statsGrid}>
            <View style={[styles.statBox, { width: '33%' }]}>
          ...
          <Text style={styles.cardTitle}>{t('congratulations')}</Text>
            ...
            <View style={styles.statsGrid}>
              <View style={styles.statBox}>
                ...
                <Text style={styles.statValue}>{new Date(pregnancy.delivered_at).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: 'numeric' })}</Text>
              </View>
              <View style={styles.statBox}>
...
const styles = StyleSheet.create({
  ...
  nextAptBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#EBF5FB', borderRadius: 10, padding: 14, marginTop: 14, marginBottom: 4, borderLeftWidth: 4, borderLeftColor: '#2980B9' },
  nextAptIcon: { fontSize: 24, marginRight: 12 },
  nextAptLabel: { fontSize: 11, color: '#2980B9', fontWeight: '600', textTransform: 'uppercase' },
  nextAptDate: { fontSize: 15, fontWeight: '700', color: '#2C3E50', marginTop: 1 },
  nextAptType: { fontSize: 13, color: '#7F8C8D', textTransform: 'capitalize', marginTop: 1 },
