...
const TRANS_NZOIA_WARDS = ['Kiminini', 'Matunda', 'Nabiswa', 'Kitale', 'Sirende', 'Hospital', 'Kwanza', 'Endebess'];
...
export default function AppointmentsScreen() {
  ...
  async function loadData() {
    try {
      ...
      if (cached.length > 0) setFacilities(cached);
    } finally {
      setLoading(false);
    }
  }
        ...
        filteredAppointments.map(apt => (
          <View key={apt.id} style={styles.aptCard}>
            <View style={styles.aptHeader}>
              ...
              <Text style={[styles.aptStatus, apt.status === 'scheduled' && styles.statusScheduled, apt.status === 'completed' && styles.statusCompleted, apt.status === 'cancelled' && styles.statusCancelled]}>
                {t(apt.status)}
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  statusScheduled: { backgroundColor: '#3498DB' },
