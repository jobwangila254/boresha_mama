...
export default function Layout({ user, setUser }) {
  ...
  const navItems = [
    ...
    { path: '/facilities', label: t('facilities'), icon: '🏥' },
    { path: '/users', label: t('users'), icon: '👥' },
    { path: '/referrals', label: t('referral_tracking'), icon: '🔄' },
    ...
    <div style={styles.container}>
      <nav style={styles.sidebar}>
        <div style={styles.logo}>
          <CountyLogo size={60} showTagline={false} />
          <div>
            <div style={styles.logoText}>{t('app_name')}</div>
...
const styles = {
  ...
  sidebar: { width: 250, background: '#004d26', color: '#fff', padding: '24px 0', display: 'flex', flexDirection: 'column' },
  ...
  navItem: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 8, color: '#AED6F1', textDecoration: 'none', fontSize: 14, marginBottom: 4 },
