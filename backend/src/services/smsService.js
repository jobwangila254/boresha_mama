...
class SmsService {
  constructor() {
    ...
    // sandbox vs live — easy to miss this toggle, learnt the hard way
  ...
  async sendSms(recipients, message) {
    if (this.provider !== 'africastalking') {
      logger.info(`SMS provider set to '${this.provider}'. Logging SMS instead of sending.`);
      const phoneNumbers = Array.isArray(recipients) ? recipients : [recipients];
      logger.info(`Log SMS to ${phoneNumbers.join(', ')}: ${message}`);
    ...
    try {
      const appointments = await db.query(
        `SELECT a.id, a.appointment_date, u.phone, u.first_name, u.id as user_id,
         ...
         WHERE DATE(a.appointment_date) = $1 AND a.status IN ('scheduled', 'confirmed') AND a.reminder_sent = false`,
        [tomorrowDate]
      );
