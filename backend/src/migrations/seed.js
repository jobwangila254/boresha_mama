...
async function seed() {
  ...
  try {
    ...
    // Seed locations (constituencies, wards, sub_locations, and villages)
    const locations = [
    ...
    const adminUserId = (await client.query("SELECT id FROM users WHERE phone = '+254700000000' LIMIT 1")).rows[0]?.id;
    ...
    for (const fs of fsFacilities) {
      ...
      const staffUser = await client.query(
        `INSERT INTO users (phone, first_name, last_name, email, password_hash, role, is_verified)
         ...
         ON CONFLICT (phone) DO NOTHING
         RETURNING id`,
        [fs.phone, fs.first, fs.last, `${fs.name.toLowerCase().replace(/\s+/g, '_')}@boreshamama.go.ke`, fsHashes[fs.name]]
      );
    ...
    // Seed CHVs (one per ward) with varied phones/passwords
    const chvData = [
    ...
    for (let ci = 0; ci < chvData.length; ci++) {
      ...
      if (chvUserId) {
        const facility = await client.query(
          `SELECT id FROM facilities WHERE ward = $1 LIMIT 1`, [c.ward]
        );
    ...
    // Seed mothers (2-3 per ward) with varied phones, DOBs, and individual passwords
    ...
    // Seed home visits for some mothers
