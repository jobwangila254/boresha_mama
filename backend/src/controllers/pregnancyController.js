...
exports.updatePregnancy = async (req, res, next) => {
  try {
    ...
    const finalRiskLevel = riskFactors ? calculateRiskLevel({}, riskFactors) : riskLevel;
    ...
    const result = await db.query(
      `UPDATE pregnancies SET
       ...
       WHERE id = $6
       RETURNING *`,
      [status, finalRiskLevel || null, riskFactors || null, facilityId, notes, req.params.id]
    );
    ...
    if ((riskLevel || riskFactors) && (updated.risk_level === 'high' || updated.risk_level === 'critical')) {
