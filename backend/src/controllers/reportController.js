    ...
    if (role === 'county_admin') {
      const statsResult = await db.query(`
        SELECT
          ...
          COUNT(*) FILTER (WHERE p.status = 'active' AND p.risk_level = 'medium')::int AS medium_risk_pregnancies,
      ...
      const totalDeliveries = await db.query(
        'SELECT COUNT(*)::int AS total FROM pregnancies WHERE delivered_at IS NOT NULL'
      );

      stats = {
        totalMothers,
        activePregnancies: r.active_pregnancies,
        mediumRiskPregnancies: r.medium_risk_pregnancies,
        ...
        totalDeliveries: totalDeliveries.rows[0].total,
      ...
      const wardRows = await db.query(`
        SELECT
          m.ward,
          COUNT(DISTINCT m.id)::int AS total_mothers,
          COUNT(DISTINCT p.id) FILTER (WHERE p.status = 'active')::int AS active_pregnancies
        FROM mothers m
        LEFT JOIN pregnancies p ON p.mother_id = m.id
        ...
        ORDER BY m.ward
      `);
      stats.wardStats = wardRows.rows.map(w => ({
        name: w.ward,
        totalMothers: w.total_mothers,
        activePregnancies: w.active_pregnancies,
        coverage: w.total_mothers > 0
          ? parseFloat(((w.active_pregnancies / w.total_mothers) * 100).toFixed(1))
          : 0,
      }));
      ...
      const activePregnancies = await db.query(
        "SELECT COUNT(*) FROM pregnancies WHERE facility_id = $1 AND status = 'active'",
        [fid]
      );
      const monthlyRegistrations = await db.query(`
        ...
        WHERE p.facility_id = $1
          AND p.created_at >= NOW() - INTERVAL '8 months'
