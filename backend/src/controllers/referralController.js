...
exports.createReferral = async (req, res, next) => {
  try {
    ...
    if (chvProfile.rows[0]) {
      fromFacilityId = chvProfile.rows[0].facility_id;
    } else {
      const staffProfile = await db.query('SELECT facility_id FROM facility_staff WHERE user_id = $1', [req.user.id]);
      fromFacilityId = staffProfile.rows[0]?.facility_id || null;
    }
